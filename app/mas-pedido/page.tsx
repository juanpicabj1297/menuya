...
import { getMostOrderedMenuItems } from "@/lib/restaurants";

export const dynamic = "force-dynamic";
export const revalidate = 0;

export default async function MostOrderedPage() {
  const products = await getMostOrderedMenuItems("suipacha", 24);
      ...
      <section className="mx-auto w-full max-w-6xl px-4 py-4 sm:px-6">
        ...
        <section className="mt-8">
          ...
          <p className="mt-3 max-w-xl text-sm leading-6 text-neutral-500 sm:text-base">
            Productos ordenados por pedidos confirmados en MenuYa. Si todavia
            faltan datos, completamos con destacados disponibles.
          </p>
        </section>
        ...
        <section className="mt-8">
          {products.length === 0 ? (
            <div className="rounded-[1.6rem] border border-dashed border-neutral-200 bg-neutral-50 p-8 text-center text-sm font-semibold text-neutral-500">
              Todavia no hay productos destacados cargados.
            </div>
          ) : (
